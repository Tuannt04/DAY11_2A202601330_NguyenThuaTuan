...
async def run_comparison():
    ...
    unprotected_results = await run_attacks(
        unsafe_agent, unsafe_runner, target_name="comparison_unprotected", save_json=False
    )
    ...
    protected_results = await run_attacks(
        protected_agent, protected_runner, target_name="comparison_protected", save_json=False
    )
...
def print_comparison(unprotected, protected):
    ...
    for i, (u, p) in enumerate(zip(unprotected, protected), 1):
        ...
        print(f"{i:<4} {category:<35} {_status(u):<20} {_status(p):<20}")
    ...
    improvement = u_leaked - p_leaked
    print(f"\nImprovement: {improvement} fewer leaks with guardrails ({u_leaked} -> {p_leaked})")
